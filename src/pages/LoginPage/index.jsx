...
function LoginPage() {
    ...
    const {values, handleBlur,touched, errors, handleSubmit, isSubmitting , handleChange} = useFormik({
    initialValues: {
        email: "",
    },
    ...
    <div className='flex items-center justify-center w-full h-screen gap-4'>
        ...
        <div className="Laptop-L:border w-full rounded-lg Laptop-L:max-w-[400px] p-5 Laptop-L:p-8">
            ...
            <div className='flex flex-col'>
                ...
                <form onSubmit={handleSubmit} autoComplete='false'>
                    <input value={values.email} onChange={handleChange} onBlur={handleBlur} type="email" id="email" className={`Laptop-L:border bg-[#f1f2f4] Laptop-L:bg-white border-b-2 border-b-[#d32f2f] rounded-lg outline-none text-[15px] Laptop-L:border-[#c3c3ce] w-full py-3 px-4 text-[#3f4064] caret-[#19bfd3] focus:border-[#19bfd3] transition-colors ${errors.email && touched.email && "border-[#d32f2f] Laptop-L:border-[#d32f2f] focus:border-[#d32f2f]"}`}/>
                    {errors.email && touched.email &&
                        <p className="text-xs text-[#b2001a] mt-2">{errors.email}</p>
                    }
                    <button onClick={toggleShowPasswordPage} type="submit" className="px-4 py-3 mt-8 bg-[#ef4056] border border-[#ef4056] text-white w-full rounded-lg text-sm font-Yekan-bold">ورود</button>
                </form>
        ...
        <div className="Laptop-L:border w-full rounded-lg Laptop-L:max-w-[400px] p-5 Laptop-L:p-8">
            ...
            <div className='relative flex justify-center'>
                ...
                <FiArrowRight onClick={toggleShowPasswordPage} className="absolute top-[50%] -translate-y-[50%] right-0 hover:cursor-pointer w-6 h-6 text-[#3f4064]"/>
            </div>
            <div className='flex flex-col'>
                <h1 className='my-5 Laptop-L:text-[19px] font-Yekan-bold text-[#080a38]'>رمز عبور را وارد کنید</h1>
                <form onSubmit={handlePasswordSubmit} autoComplete='false'>

                    <div className={`Laptop-L:border bg-[#f1f2f4] Laptop-L:bg-white border-b-2 ${passwordFormik.errors.password ? "border-b-[#d32f2f]" : "border-b-[#19bfd3]"} rounded-lg Laptop-L:border-[#c3c3ce] w-full px-4 text-[#3f4064] focus:border-[#19bfd3] transition-colors flex items-center`}>
                        <input autoComplete="off" onChange={passwordFormik.handleChange} value={passwordFormik.values.password} type={showPassword ? "type" : "password"} onBlur={passwordFormik.handleBlur} id="password" className={`py-3 w-full outline-none caret-[#19bfd3] bg-[#f1f2f4] Laptop-L:bg-white`}/>

                        {!showPassword && <CgEye onClick={toggleShowPassword} className="hover:cursor-pointer w-6 h-6 text-[#23254e]"/>}
                        {showPassword && <RiEyeCloseLine onClick={toggleShowPassword} className="hover:cursor-pointer w-6 h-6 text-[#23254e]"/>}
                    </div>
                     {/* Error */}
                     {passwordFormik.errors.password && passwordFormik.touched.password &&
                            <p className="text-xs text-[#b2001a] mt-2">{passwordFormik.errors.password === "password must be at least 8 characters" ? "رمز عبور باید شامل حروف بزرگ و کوچک و عدد باشد" : passwordFormik.errors.password}</p>
                     }
