...
function HomeFeatures() {
  ...
  return (
    <div
      className={`grid grid-cols-4 p-2 px-3 mt-6 gap-y-6 Laptop-L:flex Laptop-L:items-center Laptop-L:justify-around`}
    >
      {features.map((feature) => (
        ...
        >
          ...
          {!feature.image && (
            <div
              onClick={handleOpen}
              className="w-[52px] h-[52px] text-[#9e9fb1] bg-[#f1f2f4] rounded-full flex items-center justify-center"
            >
      ...
      <div
        onClick={handleOpen}
        className={`modal-bg fixed z-30 w-full h-screen -translate-x-1/2 -translate-y-1/2 bg-black bg-opacity-30 top-1/2 left-1/2 ${
          !openModal && 'hidden'
        } animate-fade`}
      ></div>
      <div
        className={`modal fixed z-50 bg-white Laptop-L:rounded-2xl w-full h-screen Laptop-L:pb-10 Laptop-L:max-w-[55vw] min-[1280px]:max-w-[80vh] Laptop-L:min-w-[460px] Laptop-L:max-h-[63vh] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 overflow-clip animate-fade ${
          !openModal && 'hidden'
        }`}
      >
        ...
        <div className="h-full px-5 py-4 overflow-y-scroll hell">
          <div className="mt-5">
              ...
              {featuresDetails[0].category.map((item) => (
                ...
                >
                  <img
                    src={item.image}
                    alt="icon"
                    className="w-[52px] h-[52px] hover:cursor-pointer"
                  />
                  <span className="text-[10px] Laptop-L:text-[11px] font-Yekan-bold text-[#3f4064] mt-1 hover:cursor-pointer">
                    {item.text}
                  </span>
                </div>
              ))}
            ...
            <h4 className="text-xs Laptop-L:text-sm font-Yekan-bold text-[#767790] mt-6">
            ...
            <div className="grid grid-cols-1 gap-3 mt-2 Laptop-L:grid-cols-2">
              {featuresDetails[1].services.map((service, i) => (
                <div
                  className="p-3 border-b hover:cursor-pointer Laptop-L:border border-[#e0e0e6] Laptop-L:rounded-2xl flex items-start"
                  key={i}
                >
